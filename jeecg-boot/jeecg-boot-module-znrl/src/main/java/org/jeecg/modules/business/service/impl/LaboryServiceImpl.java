...
public class LaboryServiceImpl implements ILaboryService {
    ...
    @Autowired
    public BizToolMapper bizToolMapper;
  ...
  /*  *//**
   ...
   /**
    ...
    public void submit4LabReport(SEntity sEntity){
      ...
      *  查询化验报告数据
      */
    public Page<ReportEntity> qryReportData(Page<ReportEntity> page, ReportEntity reportEntity){
        //查询本次页面的结果集
//        List<ReportEntity> list = laboryMapper.qryReportData(reportEntity);
//        return list;
        return page.setRecords(laboryMapper.qryReportData(page,reportEntity));
    }
    ...
    public List<ManualSampleEntity> qryManInfo(ManualSampleEntity manualSampleEntity) {
        List<ManualSampleEntity> list = laboryMapper.qryManInfo(manualSampleEntity);
        //设置返回结果
       /* GridModel m = new GridModel();
        m.setRows(list);*/
        return list;
