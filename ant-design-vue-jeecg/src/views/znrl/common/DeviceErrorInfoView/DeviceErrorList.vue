...
<!--          采用模糊查询-->
          ...
          <a-col :md="4" :sm="24">
            ...
            <a-form-item label="故障状态">
              <j-dict-select-tag v-model="queryParam.errorConfirm" placeholder="故障状态" dictCode="gk_device_error_confirm"/>
...
<!--    加入refs的组件 modalForm modalFormOk 成功后重新加载-->
    <device-error-modal ref="modalForm" @ok="modalFormOk"></device-error-modal>
  </a-card>
</template>
...
<script>
  ...
  //导入组件
  import DeviceErrorModal from './modules/DeviceErrorModal'
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            title:'故障状态',
            align:"center",
            dataIndex: 'errorConfirm',
            customRender:function (text) {
              return filterDictTextByCache('gk_device_error_confirm',text)
            }
          },
          {
            title:'处理详情',
        ...
        url: {
          list: "znrl/monitor/deviceErr/list",
    ...
    methods: {
      ...
      handleDeviceError: function (record) {
